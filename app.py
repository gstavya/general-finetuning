...
import math # MODIFICATION: To support cosine calculation
...
# --- MODIFICATION: Helper function for EMA Cosine Scheduling ---
...
def main():
    ...
    # MODIFICATION: Configuration for EMA scheduling
    START_EMA_DECAY = 0.96
    END_EMA_DECAY = 1.0 # Standard final value for BYOL
    ...
    # --- Datasets and DataLoaders (No changes) ---
    ...
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=DATALOADER_WORKERS, drop_last=True, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=DATALOADER_WORKERS, drop_last=False)
    test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=DATALOADER_WORKERS, drop_last=False)

    # --- Model Initialization and LR Scheduler (No changes) ---
    ...
    # --- Checkpoint Loading (No changes) ---
    start_epoch = 0
    best_val_loss = float('inf')
    os.makedirs(LOCAL_MODEL_OUTPUT_DIR, exist_ok=True)
    CHECKPOINT_CONTAINER = "resnet18"
    CHECKPOINT_BLOB_NAME = "latest_checkpoint.pth" # Load latest to resume
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        # MODIFICATION: Calculate current EMA decay for the epoch
        current_ema_decay = get_ema_decay(
            epoch, NUM_EPOCHS, START_EMA_DECAY, END_EMA_DECAY
        )
        ...
        for (view1, view2), _ in tqdm(train_loader, desc=f"Epoch {epoch+1}/{NUM_EPOCHS} [Train]"):
            ...
            # MODIFICATION: Use the calculated EMA decay for the current epoch
            update_moving_average(target_network, online_network, decay=current_ema_decay)
        
        avg_train_loss = total_train_loss / len(train_loader)

        # --- Validation Phase (No changes in logic) ---
        online_network.eval()
        ...
        # --- Checkpoint Saving Logic (No changes) ---
        latest_checkpoint_path = os.path.join(LOCAL_MODEL_OUTPUT_DIR, "latest_checkpoint.pth")
        torch.save({
            'epoch': epoch, 'online_network_state_dict': online_network.state_dict(),
            'target_network_state_dict': target_network.state_dict(), 'prediction_head_state_dict': prediction_head.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(), 'scheduler_state_dict': scheduler.state_dict(),
            'train_loss': avg_train_loss, 'val_loss': avg_val_loss, 'best_val_loss': best_val_loss
    ...
    # --- Save Final Backbone (No changes) ---
